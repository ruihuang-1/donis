...
params = {
    ...
    "fixed_grid": False,
    "grid_size": 100,  # determines grid size of isc
    "interp_type": "cubic",  # options: cubic, idw, wind, krig
    "isc_eval_num": 500,  # seed num. actual num may change after random sampling
    "do_end_isc": False,
    "isc_end_step": 5000,  # determined by do_end_isc

    "isf_eval_num": 100,  # actual num may change after random sampling
    "do_reweighting": True,
    ...
    "branch": [128, 128, 128, 128, 128, 128],
...
def main():
    ...
    # Function Space
    func_space = dde.data.GRF(length_scale=2, kernel='ExpSineSquared')
    ...
    samples = data['U'].astype(np.float32)
    X, T = np.meshgrid(data['x'], data['t'], indexing='ij')
    val_x = np.stack((X, T), axis=-1).reshape(-1, 2).astype(np.float32)
    val_v = samples[:, :, 0]
    val_u = samples.reshape(params['num_val'], -1)
    ...
    for epoch in pbar:
        ...
        if params['enable_isc'] and epoch < isc_end_step:
            new_grid, col_dom, isc_weights = pts_sampling_v(net, pde, params['num_dom'], grid, func_idx, isc_eval_xs,
                                                            train_vals, interp=params['interp_type'],
        ...
        # TRAIN
        train_y_ic = interps_eval(train_interps[func_idx], train_x_ic[:, 0])
        train_pde, train_ic, train_bc = train_step(net, optimizer, train_vals[func_idx], col_pts, weights, train_y_ic)
        # TEST
        test_y_ic = interps_eval(test_interps, test_x_ic[:, 0])
        test_pde, test_ic, test_bc = test_step(net, test_vals, test_x, test_y_ic)

        # EXTRA_TEST
        if epoch % 100 == 0:  # evaluate train loss with uniform grid
            extra_y_ic = interps_eval(train_interps, train_x_ic[:, 0]).reshape(len(train_interps), -1)
            extra_pde, extra_ic, extra_bc = extra_test_step(net, train_vals, train_x, extra_y_ic, params['num_func'],
                                                            params['funcs_batch'])
...
def pde_extra(x, y, aux=None):
    # returns pde residual and extra gradient info
    dy_x = dde.grad.jacobian(y, x, i=0, j=0)
    dy_t = dde.grad.jacobian(y, x, i=0, j=1)
    dy_xx = dde.grad.hessian(y, x, i=0, j=0)
    return dy_t - params['epsilon'] ** 2 * dy_xx ** 2 + (y ** 3 - y), dy_x

def pde(x, y, aux=None):
    dy_x = dde.grad.jacobian(y, x, i=0, j=0)
    dy_t = dde.grad.jacobian(y, x, i=0, j=1)
    dy_xx = dde.grad.hessian(y, x, i=0, j=0)
    return dy_t - params['epsilon'] ** 2 * dy_xx ** 2 + (y ** 3 - y)
...
def train_step(net, optimizer, train_vals, col_pts, weights, target_ic):
    net.train()
    optimizer.zero_grad()

    loss_pde, loss_ic, loss_bc = get_losses(net, train_vals, col_pts, weights, target_ic, params['do_reweighting'])
    loss = loss_bc + loss_ic + loss_pde
    loss.backward()

    # Gradient clipping
    if params['enable_grad_clip']:
        torch.nn.utils.clip_grad_norm_(net.parameters(), max_norm=params['clip_norm'])
...
def test_step(net, test_vals, test_x, test_y_ic):
    net.eval()
    loss_pde, loss_ic, loss_bc = get_losses(net, test_vals, test_x, None, test_y_ic, False, True)
    test_pde, test_ic, test_bc = loss_pde.item(), loss_ic.item(), loss_bc.item()
    dde.grad.clear()
    return test_pde, test_ic, test_bc


def extra_test_step(net, test_vals, test_x, test_y_ic, num_all, num_batch):
...
def val_step(net, val_u, val_x, val_v):
    ...
    x = np.tile(val_x[None, :, :], (params['num_val'], 1, 1)).astype(np.float32)  # B*P*d1
    v = np.tile(val_v[:, None, :], (1, x.shape[1], 1)).astype(np.float32)  # B*P*d2
    x = torch.tensor(x.reshape(-1, 2), requires_grad=False)
    v = torch.tensor(v.reshape(-1, params['branch'][0]), requires_grad=False)
    output = net.forward((v, x)).reshape(params['num_val'], -1).detach()
    mse = mean_square_error(target, output)
    l2re = mean_l2_relative_error(target, output)
    return mse.item(), l2re.item()


def get_losses(net, func_vals, col_pts, weights, target_ic, reweighting, is_test=False):
    ...
    # pde loss
    error_pdes, dy_x = pde_extra(x, output)
    ...
    target_ic = torch.tensor(target_ic).reshape(funcs_batch, -1)
    loss_ic = mean_square_error(output_ic, target_ic)
    # bc loss
    # periodic bc
    output_bc_l = output.reshape(funcs_batch, -1)[:, -num_bc:-num_bc // 2]
    output_bc_r = output.reshape(funcs_batch, -1)[:, -num_bc // 2:]
    err_bc = output_bc_l - output_bc_r

    output_bc_grad_l = dy_x.reshape(funcs_batch, -1)[:, -num_bc:-num_bc // 2]
    output_bc_grad_r = dy_x.reshape(funcs_batch, -1)[:, -num_bc // 2:]
    err_bc_grad = output_bc_grad_l - output_bc_grad_r
    loss_bc = mean_square_error(err_bc, torch.zeros_like(err_bc)) + mean_square_error(err_bc_grad,
                                                                                      torch.zeros_like(err_bc_grad))
